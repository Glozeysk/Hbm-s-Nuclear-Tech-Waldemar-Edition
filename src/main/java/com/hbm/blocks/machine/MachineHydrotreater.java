...
//Ported from NTM:CE (Warfactory-Official/Hbm-s-Nuclear-Tech-CE): MachineHydrotreater
public class MachineHydrotreater extends BlockDummyable {
	...
	public TileEntity createNewTileEntity(World world, int meta) {
		if(meta >= 12)
			return new TileEntityMachineHydrotreater();
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		if(world.isRemote) {
			...
			if(entity instanceof TileEntityMachineHydrotreater) {
				player.openGui(MainRegistry.instance, ModBlocks.guiID_machine_hydrotreater, world, pos1[0], pos1[1], pos1[2]);
			}
			return true;
		} else {
			return false;
